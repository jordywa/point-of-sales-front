...
import { Search, ScanLine, Menu, History, User, Trash2, Edit2, X, ShoppingCart, Calculator, QrCode, ArrowLeft, Check, LayoutGrid, List, ChevronDown, Plus, Truck, Calendar, Mail, Printer, Pencil, LogOut, Key, ChevronUp, ChevronLeft, ChevronRight } from 'lucide-react';
...
const KasirPage: React.FC<KasirPageProps> = ({ setIsSidebarOpen, formatRupiah }) => {
  ...
  // --- State Draft & History ---
  // WOI BACKEND JORDY: Fetch data Draft Transaction yang statusnya 'PENDING' dari database saat halaman dimuat
  ...
  const startItemIndex = filteredProducts.length === 0 ? 0 : indexOfFirstItem + 1;
  const endItemIndex = Math.min(indexOfLastItem, filteredProducts.length);
  ...
  const totalDrafts = draftTransactions.reduce((acc, draft) => acc + draft.total, 0);
  ...
  const handleLogout = () => {
    if (confirm("Apakah Anda yakin ingin logout?")) {
      // #WOIJORDY BACK END WOI: Panggil API Logout / Invalidate Token di sini
  ...
  const handleSaveDraft = () => {
    ...
    if (activeDraftId !== null) {
      // WOI BACKEND JORDY: UPDATE data draft di database berdasarkan ID
      // PUT /api/drafts/{activeDraftId} dengan body: newDraft
      setDraftTransactions(prev => prev.map(d => d.id === activeDraftId ? newDraft : d));
    } else {
      // WOI BACKEND JORDY: INSERT data draft baru ke database
      // POST /api/drafts dengan body: newDraft
  ...
  const handleDeleteDraft = (e: React.MouseEvent, id: number) => {
    ...
    if (window.confirm("Hapus draft ini?")) {
      // WOI BACKEND JORDY: DELETE data draft dari database
  ...
  const handleFinalizePO = () => {
    ...
    // WOI BACKEND JORDY: INSERT Transaksi baru tipe 'KREDIT' (PO)
    // POST /api/sales-orders dengan body: { type: 'KREDIT', invoiceNumber: currentInvoiceNo, items: cart, customer: selectedCustomer, jatuhTempo: jatuhTempoDate, total: subTotal }
    const isNewTransaction = activeDraftId === null;
    if (activeDraftId !== null) {
  ...
  const handleFinishTransaction = () => {
    // WOI BACKEND JORDY: INSERT Transaksi baru tipe 'TUNAI/LUNAS'
    // POST /api/sales-orders dengan body: { type: 'TUNAI', invoiceNumber: currentInvoiceNo, items: cart, customer: selectedCustomer, paymentMethod: selectedPaymentMethod, paymentAmount: paymentAmount, total: subTotal, change: kembalian }
    const isNewTransaction = activeDraftId === null;
    if (activeDraftId !== null) {
      setDraftTransactions(prev => prev.filter(d => d.id !== activeDraftId));
    }
  ...
  const decrementQty = (id: number) => {
    ...
    // Check if qty will become 0
    if (item.qty === 1) {
      // Check if qtyPO also 0 or not exists (meaning total qty will be 0)
      const totalQtyPO = item.qtyPO || 0;
      if (totalQtyPO === 0) {
  ...
  const decrementQtyPO = (id: number) => {
    ...
    // Check if qtyPO will become 0
    if (currentQtyPO === 1) {
      // Check if qty is also 0 (meaning total qty will be 0)
      if (item.qty === 0) {
        // Show confirmation dialog
        setConfirmDialogConfig({
          title: "Konfirmasi Hapus",
          message: `Qty PO akan menjadi 0. Produk "${item.name}" akan dihapus dari list. Lanjutkan?`,
          onConfirm: () => {
            const newCart = cart.filter(it => it.id !== id);
            setCart(newCart);
            if (newCart.length === 0) setActiveDraftId(null);
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full w-full overflow-hidden relative bg-gray-50">
      ...
      <div className={`w-full lg:flex-1 flex flex-col h-full border-r border-gray-300 relative z-0 transition-all ${isMobileCartOpen ? 'hidden lg:flex' : 'flex'}`}>
        ...
        <div className="bg-white border-t border-gray-200 p-4 flex justify-between items-center sticky bottom-0 z-20 shadow-2xl">
          <div className="text-gray-800 font-medium">
            Menampilkan <span className="font-bold">{startItemIndex} - {endItemIndex}</span> dari <span className="font-bold">{filteredProducts.length}</span> Produk
          </div>
      ...
      <div className={`w-full lg:w-[500px] xl:w-[600px] bg-white flex-col h-full shadow-2xl z-10 border-l border-gray-200 transition-all ${isMobileCartOpen ? 'flex fixed inset-0 z-50 lg:static' : 'hidden lg:flex'}`}>
            ...
            {cart.length === 0 ? <div className="flex items-center justify-center h-40 text-xs text-gray-400 italic">Belum ada transaksi</div> : (
              [...cart].sort((a, b) => a.name.localeCompare(b.name)).map((item) => {
                ...
                return transactionMode === 'PO' ? (
                      ...
                      <div className="flex-1 pr-4">
                        ...
                        {editingNoteId === item.id ? (
                          <div className="flex gap-1 mt-1 animate-in fade-in duration-200">
                            ...
                            />
                            <button onClick={() => saveEditNote(item.id)} className="text-green-600 hover:bg-green-100 p-1 rounded"><Check className="w-4 h-4" /></button>
                          </div>
                        ) : (
                      ...
                      <div className="flex flex-col items-center min-w-[90px]">
                        ...
                        {originalProduct ? (
                          <select
                            ...
                            value={item.unit}
                            onChange={(e) => handleCartUnitChange(item.id, item.productId, e.target.value)}
                          >
                  ...
                  // HORIZONTAL LAYOUT untuk mode biasa
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200 p-4">
          <div className="bg-white rounded-3xl w-full max-w-[500px] p-6 shadow-2xl relative border border-gray-200">
            ...
            <div className="space-y-3 mb-8 max-h-60 overflow-y-auto">
              {filteredCustomers.length > 0 ? (filteredCustomers.map(cust => (<div key={cust.id} className="flex items-center justify-between"><div className="flex gap-4 text-gray-800"><span className="font-medium w-24 truncate">{cust.name}</span><span className="text-gray-600 w-32 hidden sm:inline">{cust.phone}</span><span className="text-gray-600">{cust.points} Poin</span></div><button onClick={() => handleSelectCustomer(cust)} className="bg-[#5EEAD4] text-black font-medium px-6 py-1 rounded-full hover:bg-teal-300 shadow-sm">Choose</button></div>))) : (<div className="text-center text-gray-500 py-4">Customer tidak ditemukan</div>)}
            </div>
